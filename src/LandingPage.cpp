...
bool DrawLandingPage(int sw, int sh, int titleSize, int fontSize,
    ...
    AppTheme themePreviews[] = {AppTheme::Dark(), AppTheme::Light(), AppTheme::HighContrast()};
    ...
    int btnW = static_cast<int>(260 * cardScale);
    int btnH = static_cast<int>(56 * cardScale);
    ...
    if (btnFont < 14) btnFont = 14;
    int btnTextW = MeasureText(btnText, btnFont);
    if (btnTextW > btnW - 16) {
        btnFont = static_cast<int>(btnFont * 0.85f);
        if (btnFont < 12) btnFont = 12;
        btnTextW = MeasureText(btnText, btnFont);
    }
